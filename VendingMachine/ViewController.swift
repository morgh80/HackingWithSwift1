...
class ViewController: UIViewController, UICollectionViewDataSource, UICollectionViewDelegate {
    ...
    var currentSeletion: VendingSelection?
    ...
    @IBAction func purchase() {
        if let currentSeletion = currentSeletion {
            do {
                try vendingMachine.vend(selection: currentSeletion, quantity: Int(quantityStepper.value))
                updateDisplayWith(balance: vendingMachine.amountDeposited, totalPrice: 0.0, itemPrice: 0, itemQuantity: 1)
            } catch VendingMachineError.outOfStock {
                showAlert(title: "Out of stock", message: "Sorry, the item is currently unavailable")
            } catch VendingMachineError.invalidSelection {
                showAlert(title: "Invalid selection", message: "Choose another item")
    ...
    @IBAction func updateQuantity(_ sender: UIStepper) {
        ...
        if let currentSeletion = currentSeletion, let item = vendingMachine.item(forSelection: currentSeletion) {
    ...
    //First, make the showAlert method more configurable so that you can specify a title, message, and style when you call it. Next, you need to account for every error that we've thrown in the vending machine error type. And for each error, present an alert view that's configured to that error.
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        currentSeletion = vendingMachine.selection[indexPath.row]
